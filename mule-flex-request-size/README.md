...
4. This will generate a file named _mule_flex_request_size.wasm_ in the folder _target/wasm32-unknown-unknown/release_ . You can optionally use wasm-gc to reduce its size. If necessary, you can use an AWS S3 bucket to copy that file so it will be available on your laptop to upload it to MuleSoft Exchange using a browser.
...
1. Upload the file _mule_flex_request_size.wasm_ you obtained in previous steps after compiling the sources using Rust and Cargo
